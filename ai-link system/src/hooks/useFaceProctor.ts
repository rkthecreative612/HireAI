...
export function useFaceProctor(
          ...
          if (!detectedViolation) {
            ...
            if (currentViolatingTypeRef.current !== detectedViolation) {
              ...
              if (duration >= debounceViolationMs && timeSinceLastStrike >= strikeCooldownMs) {
                ...
                const nextStrike = Math.min(3, currentStrikesRef.current + 1);
                ...
                if (nextStrike >= 3 && onMaxStrikesReached) {
                  onMaxStrikesReached();
                }
              }
            }
  ...
  }, [isEnabled, cameraActive, debounceViolationMs, strikeCooldownMs, onStrike, onMaxStrikesReached, videoRef]);
