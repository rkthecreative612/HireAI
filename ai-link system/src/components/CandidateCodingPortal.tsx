...
export const CandidateCodingPortal: React.FC<CandidateCodingPortalProps> = ({
  ...
  const handleSubmitAssessment = async () => {
    ...
    // 1. Notify parent reports store so it appears in Interviewer Reports view
    if (onSaveReport) {
      onSaveReport(report);
    }

    // 2. Persist completed session to Supabase
    ...
    if (token) {
      updateCodingAssessmentStatus(token, 'completed').catch((err) => {
        console.warn('Could not update assessment status in Supabase:', err);
      });
    }
  };
