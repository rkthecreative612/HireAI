...
import { InterviewReport, CodingReportData } from '../types';
import { 
  Award, 
  CheckCircle2, 
  AlertTriangle, 
...
export const InterviewReportView: React.FC<InterviewReportViewProps> = ({
  ...
  // Derive hybridData safely if report.hybridReportData is present or can be inferred from questionSessions
  const hybridData = report.hybridReportData || (() => {
    if (report.questionSessions && report.questionSessions.length > 0) {
      ...
      if (mcqQs.length > 0) {
        ...
        const mcqResults = mcqQs.map((qs) => {
          const candAns = qs.candidateAnswer || '';
          const isAttended = Boolean(candAns) && candAns !== '(Unattempted)';
          if (isAttended) attempted++;
          
          const options = qs.question.options || [];
          const correctIdx = options.indexOf(qs.question.correctAnswer || '');
          const candIdx = isAttended ? options.indexOf(candAns) : -1;
          
          const isCorr = qs.evaluation?.score === 100 || (correctIdx >= 0 && candIdx === correctIdx);
          if (isCorr) correct++;
          ...
          return {
            ...
            candidateAnswerIndex: candIdx >= 0 ? candIdx : null,
            isCorrect: isCorr,
            difficulty: qs.question.difficulty
          };
        });

        const fallbackCodingReport: CodingReportData = report.codingReportData || {
          totalQuestions: 0,
          attemptedQuestions: 0,
          solvedQuestions: 0,
          partiallySolvedQuestions: 0,
          failedQuestions: 0,
          totalTestCases: 0,
          passedTestCases: 0,
          failedTestCases: 0,
        ...
        return {
          ...
          mcqCorrect: correct,
          mcqWrong: wrong,
          mcqScorePercent: mcqQs.length > 0 ? Math.round((correct / mcqQs.length) * 100) : 0,
          mcqTimeSpentSeconds: 0,
          mcqResults,
          codingReportData: fallbackCodingReport
        };
      }
    }
    return undefined;
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-black/40 p-3.5 rounded-2xl border border-white/5">
            ...
            <div className="p-2.5 rounded-xl bg-purple-500/5 border border-purple-500/20 space-y-0.5">
              ...
              <div className="text-lg font-mono font-bold text-white">
                {cData?.totalTestCases
                  ...
                  : (cData?.solvedQuestions && cData.solvedQuestions > 0 ? 100 : 0)}%{' '}
            ...
            <div className="p-2.5 rounded-xl bg-white/5 border border-white/5 space-y-0.5">
              ...
              <div className="text-sm font-mono font-bold text-slate-200 mt-1">
                {Math.round((hybridData?.mcqScorePercent ?? 0) * 0.4)} + {Math.round(
                  (cData?.totalTestCases
                    ? Math.round(((cData?.passedTestCases ?? 0) / cData.totalTestCases) * 100)
                    : (cData?.solvedQuestions && cData.solvedQuestions > 0 ? 100 : 0)) * 0.6
                )}
              </div>
              <p className="text-[10px] text-slate-400 font-mono">
