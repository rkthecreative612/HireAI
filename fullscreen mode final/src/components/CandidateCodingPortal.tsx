...
export const CandidateCodingPortal: React.FC<CandidateCodingPortalProps> = ({
  ...
  const handleStart = () => {
    ...
    if (token) {
      updateCodingAssessmentStatus(token, 'in_progress').catch((err) => {
        console.warn('Could not update coding assessment status to in_progress:', err);
      });
    }
  ...
  // View: Single-Use Assessment Already Completed
  if (isAlreadyCompleted) {
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-6 text-center animate-fade-in">
        <div className="max-w-md w-full bg-[#0F0F12] border border-amber-500/30 rounded-2xl p-8 space-y-5 shadow-2xl">
          <div className="w-14 h-14 rounded-2xl bg-amber-500/10 border border-amber-500/25 text-amber-400 flex items-center justify-center mx-auto shadow-lg">
          ...
          {assessment && (
            <div className="p-4 rounded-xl bg-black/40 border border-white/5 text-left space-y-2 text-xs font-mono">
              <div className="flex items-center justify-between text-slate-400">
                <span>Assessment:</span>
                <span className="text-white font-sans font-semibold truncate max-w-[200px]">{assessment.set_name}</span>
              </div>
              {assessment.candidate_name && (
                <div className="flex items-center justify-between text-slate-400">
                  <span>Candidate:</span>
                  <span className="text-indigo-300 font-sans">{assessment.candidate_name}</span>
                </div>
              )}
              <div className="flex items-center justify-between text-slate-400">
                <span>Status:</span>
          ...
          <p className="text-[11px] text-slate-500 leading-normal">
            Your code submissions and proctoring events have been safely recorded. If you encountered any unexpected issues, please contact your hiring recruiter.
          </p>

          {onReturnToHome && (
            <button
              onClick={onReturnToHome}
              className="w-full py-2.5 bg-white/10 hover:bg-white/15 text-white rounded-xl text-xs font-semibold transition-all cursor-pointer"
            >
              Return to Home
            </button>
          )}
