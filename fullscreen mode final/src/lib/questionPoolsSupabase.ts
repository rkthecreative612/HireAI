...
export async function fetchInterviewReportsFromDb(): Promise<InterviewReport[] | null> {
  ...
  try {
    ...
    return data.map((row: any) => {
      const rawSessions = Array.isArray(row.question_sessions) ? row.question_sessions : [];
      let metaObj: any = {};
      ...
      return {
        ...
        roleName: row.role_name,
        experienceLevel: row.experience_level,
        assessmentType: row.assessment_type || (metaObj.hybridReportData ? 'hybrid' : 'descriptive'),
        ...
        questionSessions: cleanSessions,
        totalTimeSpentSeconds: Number(row.total_time_spent_seconds || 0),
        completedAt: row.completed_at || new Date().toISOString(),
        hybridReportData: row.hybrid_report_data || row.hybridReportData || metaObj.hybridReportData,
        codingReportData: row.coding_report_data || row.codingReportData || metaObj.codingReportData,
        security_events: row.security_events || metaObj.security_events || [],
        proctor_events: row.proctor_events || metaObj.proctor_events || [],
        security_strikes_count: row.security_strikes_count ?? metaObj.security_strikes_count ?? 0,
        proctor_strikes_count: row.proctor_strikes_count ?? metaObj.proctor_strikes_count ?? 0,
      };
    });
  } catch (err) {
...
export async function saveInterviewReportToDb(report: InterviewReport): Promise<boolean> {
  ...
  try {
    const rawSessions = [...(report.questionSessions || [])];
    if (report.hybridReportData || report.codingReportData) {
      rawSessions.push({
        _hybrid_meta: true,
        hybridReportData: report.hybridReportData,
        codingReportData: report.codingReportData,
        security_events: report.security_events || [],
        proctor_events: report.proctor_events || [],
        security_strikes_count: report.security_strikes_count || 0,
        proctor_strikes_count: report.proctor_strikes_count || 0
      } as any);
    }
