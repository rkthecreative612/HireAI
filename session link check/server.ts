...
6. followUpQuestionsForInterviewer: 3-4 concise probing questions for the live round.`;
      ...
      let progressionBonusUnlocked = false;

      if (isMcq) {
        rawTechnicalScore = mcqCalculatedScore;
        progressionBonus = 0;
        progressionBonusUnlocked = false;
      } else {
        const baseScore = Math.min(
          ...
          Math.max(
            ...
            Math.round(
              parsed?.overallScore ??
                categoryBreakdown.reduce((a, c) => a + c.score, 0) / Math.max(1, categoryBreakdown.length)
            )
          )
        );
        rawTechnicalScore = baseScore;

        // Calculate Hard Questions reached in Descriptive mode
        const hardQuestionsCount = questionSessions.filter((s: any) => s.question?.difficulty === 'hard').length;
        const totalQs = questionSessions.length;
        ...
        if (hardQuestionsCount >= requiredHard) {
          progressionBonusUnlocked = true;
          progressionBonus = bonusAmount;
        }
      }

      const finalScore = isMcq
        ? mcqCalculatedScore
        : Math.min(100, rawTechnicalScore + progressionBonus);
  ...
  // Helper: Run Python code locally via CLI
  function runPythonLocally(script: string, stdin: string): Promise<{ output: string; cpuTime: string; memory: string }> {
    return new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        pyProcess.kill();
        reject(new Error('Execution timed out (5 seconds limit exceeded)'));
      }, 5000);
  ...
  // Helper: Run Java code via Gemini execution simulator
  async function runJavaWithGemini(script: string, stdin: string): Promise<{ output: string; cpuTime: string; memory: string }> {
    const prompt = `You are a strict Java 17 JVM compiler and runtime simulator.
Execute the following Java code against the provided Standard Input (stdin).
...
Output requirements:
- Return ONLY the exact output (STDOUT) that executing this Java code with the given stdin would print to console.
- Do NOT wrap in markdown code blocks (\`\`\` or similar).
- Do NOT include explanations, disclaimers, or intro/outro notes.
- If the code has syntax errors or runtime exceptions, output the standard Java compiler error or Exception message.`;
  ...
  // Endpoint to execute Java / Python code
  app.post('/api/coding/execute', async (req, res) => {
    try {
      ...
      // 1. If Python, execute locally using native python3 CLI for instant execution
      if (isPython) {
        try {
          const result = await runPythonLocally(script, stdin || '');
          return res.json(result);
        } catch (pyErr: any) {
          console.warn('Local python execution failed, attempting fallback:', pyErr);
        }
      }

      // 2. Attempt JDoodle API
      const selectedLanguage = isPython ? 'python3' : 'java';
      const clientId = process.env.JDOODLE_CLIENT_ID || 'f3afa016d94333a77c1a1c3c9eaf91b8';
      const clientSecret = process.env.JDOODLE_CLIENT_SECRET || 'ae72aceee6003177b047e087450a213dd985f1bc28db881b31ad543a88cf432';
      ...
      try {
        ...
        if (response.ok) {
          ...
          // Verify data doesn't contain quota/limit error object
          if (data && (data.statusCode === 429 || data.statusCode === 401 || data.statusCode === 400 || (data.error && String(data.error).includes('limit')))) {
            console.warn('JDoodle returned API limit or auth error:', data);
          } else if (data && (data.output !== undefined || data.error !== undefined)) {
            return res.json(data);
          }
        } else {
          console.warn(`JDoodle returned HTTP status ${response.status}`);
        }
      } catch (jdErr) {
        console.warn('JDoodle API call failed:', jdErr);
      }

      // 3. Fallback: AI Sandbox Execution Engine
      if (isPython) {
        const result = await runPythonLocally(script, stdin || '');
        return res.json(result);
      } else {
        const javaResult = await runJavaWithGemini(script, stdin || '');
