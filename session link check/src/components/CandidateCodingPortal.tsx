...
export const CandidateCodingPortal: React.FC<CandidateCodingPortalProps> = ({
  ...
  const handleSubmitAssessment = async () => {
    ...
    try {
      ...
      if (!saveRes.success) {
        setSaveStatus({ isSaving: false, error: saveRes.error || 'Failed to save report to Supabase database.' });
      } else {
        setSaveStatus({ isSaving: false, success: true });
      }
    } catch (err: any) {
      console.warn('Could not save completed coding session to Supabase:', err);
      setSaveStatus({ isSaving: false, error: err?.message || 'Unexpected error communicating with Supabase.' });
    }
  };
  ...
  if (isSubmitted && finalReport) {
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-6 text-center">
        <div className="max-w-lg w-full bg-[#0F0F12] border border-emerald-500/30 rounded-2xl p-8 space-y-6 shadow-2xl">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-slate-300 leading-relaxed">
              Thank you, <strong className="text-white">{finalReport.candidateName}</strong>. Your code solutions and test results have been securely recorded and transmitted to the hiring team.
            </p>
          </div>
          ...
          {saveStatus.isSaving && (
            <div className="flex items-center justify-center space-x-2 text-indigo-400 text-xs py-2 bg-indigo-500/10 rounded-xl border border-indigo-500/20">
              ...
              <span>Syncing report to Supabase cloud database...</span>
            </div>
          )}
          ...
          {saveStatus.error && (
            <div className="p-3 bg-rose-500/10 border border-rose-500/30 rounded-xl text-xs text-rose-300 text-left space-y-1">
              <p className="font-bold flex items-center space-x-1.5 text-rose-200">
                ...
                <span>Cloud Database Sync Notice</span>
              </p>
          ...
          {saveStatus.success && (
            <div className="flex items-center justify-center space-x-1.5 text-emerald-400 text-xs py-2 bg-emerald-500/10 rounded-xl border border-emerald-500/20">
              <CheckCircle2 className="w-4 h-4" />
              <span>Report securely synced to Supabase database.</span>
            </div>
          )}
          ...
          <div className="grid grid-cols-3 gap-3 bg-white/5 p-4 rounded-xl border border-white/10 text-left font-mono">
            <div>
              ...
              <span className="text-lg font-bold text-emerald-400">{finalReport.overallScore}%</span>
            </div>
            <div>
