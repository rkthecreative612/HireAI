...
import {
  ...
  saveHybridSetToStorage,
  deleteHybridSetFromStorage,
...
export const InterviewModeView: React.FC<InterviewModeViewProps> = ({
  ...
  const loadSets = () => {
    const loaded = fetchHybridSetsFromStorage();
    setSets(loaded);
  };

  const handleSaveSet = (set: HybridQuestionSet) => {
  ...
  const handleConfirmDelete = () => {
    ...
    deleteHybridSetFromStorage(setToDelete.id);
    setSets((prev) => prev.filter((s) => s.id !== setToDelete.id));
    setSetToDelete(null);
  };

  const handleDuplicateSet = (set: HybridQuestionSet) => {
    ...
    saveHybridSetToStorage(duplicated);
    loadSets();
  };
