...
export const HybridCandidatePortal: React.FC<HybridCandidatePortalProps> = ({
  ...
  const handleFinalSubmit = async (
    ...
    const codingScorePercent =
      totalTestCases > 0 ? Math.round((passedTestCases / totalTestCases) * 100) : 100;
    ...
    const report: InterviewReport = {
      ...
      roleName: `${cleanedSetName} (MCQ + Coding)`,
      experienceLevel: 'Mid',
    ...
    try {
      const sessionRecord: CodingSessionRecord = {
        ...
        set_id: assessment.setId,
        set_name: cleanedSetName,
        duration_minutes: Math.ceil(
          (assessment.mcqDurationSeconds + assessment.codingDurationSeconds) / 60
        ),
        time_spent_seconds: report.totalTimeSpentSeconds,
        overall_score: finalOverallScore,
        status: isTerminated ? 'terminated_cheating' : 'completed',
        ...
        proctor_termination_reason: proctorTerminationReason,
        proctor_strikes_count: finalEvents.length,
        created_at: new Date(Date.now() - report.totalTimeSpentSeconds * 1000).toISOString(),
        completed_at: new Date().toISOString()
      };

      await saveCodingSessionToDb(sessionRecord);
      if (token) {
        await updateCodingAssessmentStatus(token, 'completed');
        await updateHybridAssessmentStatus(token, 'completed');
      }
    } catch (err) {
      console.warn('Could not save hybrid session to DB:', err);
    }
