          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center space-x-3 cursor-pointer" onClick={() => handleNavClick('select_role')}>
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-500 font-normal">Interview AI Agent</p>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-2 sm:space-x-3">
              {selectedPool && (
                <div className="hidden xl:flex items-center space-x-2 px-2.5 py-1 rounded-lg bg-white/5 text-slate-300 text-xs border border-white/10 font-mono">
                  <span className="w-2 h-2 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]" />
                  <span className="font-medium text-white truncate max-w-[150px]">{selectedPool.roleName}</span>
                </div>
              )}

              {/* Recruiter User & Sign Out */}
              {userEmail && onSignOut && (
                <div className="flex items-center space-x-2 border-l border-white/10 pl-2 sm:pl-3">
                  <div className="flex items-center space-x-1.5 text-xs text-slate-300 font-mono px-2.5 py-1 rounded-lg bg-white/5 border border-white/10">
                    <User className="w-3.5 h-3.5 text-indigo-400 shrink-0" />
                    <span className="truncate max-w-[140px] text-white font-medium">{userEmail}</span>
                  </div>
                  <button
                    type="button"
                    onClick={onSignOut}
                    className="p-1.5 text-slate-400 hover:text-rose-300 hover:bg-rose-500/10 rounded-lg transition-all border border-white/5 hover:border-rose-500/20 cursor-pointer"
                    title="Sign Out"
                  >
