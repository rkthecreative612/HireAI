...
export const InterviewKiosk: React.FC<InterviewKioskProps> = ({
  ...
  // Submit Answer & Evaluate via Gemini
  const handleSubmitAnswer = async (e: React.FormEvent) => {
    ...
    try {
      // Evaluate answer via Gemini API server
      ...
      if (!res.ok) {
        ...
        if (res.status === 429 || errMsg.includes('429') || errMsg.includes('quota') || errMsg.includes('rate limit')) {
          errMsg = 'Gemini API Rate Limit / Quota Exceeded (429): Token quota resets automatically every 60 seconds (1 minute). Please wait ~30 seconds and click "Retry Submit".';
        }
      ...
      let msg = err?.message || 'Error communicating with Gemini AI.';
      if (err?.name === 'AbortError') {
        msg = 'Evaluation timed out after 25s. Gemini server took too long to respond. Please try clicking "Retry Submit".';
      }
        ...
        <div className="bg-[#0F0F12] border border-white/10 rounded-3xl p-8 sm:p-10 space-y-8 shadow-2xl text-slate-200">
          <div className="text-center space-y-3">
            ...
            <p className="text-slate-400 text-xs tracking-wide">
              Technical Assessment for <span className="text-indigo-400 font-semibold">{pool.roleName}</span> ({pool.experienceLevel})
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="bg-white/5 p-4 rounded-2xl border border-white/5 space-y-2">
              <div className="flex items-center space-x-2 text-indigo-400 font-bold text-xs uppercase tracking-widest">
                ...
                <span>{targetCount} Adaptive Questions</span>
              </div>
