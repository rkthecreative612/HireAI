...
export const CodingRoundView: React.FC<CodingRoundViewProps> = ({
  ...
  // Java Question Sets State (Default static presets fallback, overwritten by Supabase)
  const [javaSets, setJavaSets] = useState<CodingQuestionSet[]>(DEFAULT_JAVA_SETS);

  // Python Question Sets State (Default static presets fallback, overwritten by Supabase)
  const [pythonSets, setPythonSets] = useState<CodingQuestionSet[]>(DEFAULT_PYTHON_SETS);
  ...
  // AI Proctor Mode State
  const [isProctorModeEnabled, setIsProctorModeEnabled] = useState<boolean>(true);
  ...
  const [hasCopiedMigrationSql, setHasCopiedMigrationSql] = useState<boolean>(false);
  ...
  const handleCreateAssessmentLink = async () => {
    ...
    try {
      ...
      const assessmentRecord: CodingAssessmentRecord = {
        ...
        proctor_enabled: isProctorModeEnabled,
        status: 'active',
  ...
  // Handle Proctor Max Strikes Termination (4th Strike - Immediate Assessment End)
  const handleProctorTermination = async (lastEvent?: any, allEventsList?: any[]) => {
    if (isRoundSubmitted || isProctorTerminated) return;
    ...
    // Calculate halfway test case scores at moment of termination
    ...
    const questionResultsList: QuestionCodingResult[] = questions.map((q) => {
      ...
      totalTestCasesCount += qTotal;
      passedTestCasesCount += qPassed;

      let status: 'Solved' | 'Partial' | 'Failed' | 'Not Attempted' = 'Not Attempted';
      let score = 0;

      if (!isCodeModified && qPassed === 0) {
        status = 'Not Attempted';
        unattemptedCount++;
      } else if (qPassed === qTotal && qTotal > 0) {
        status = 'Solved';
        score = 100;
        solvedCount++;
      } else if (qPassed > 0) {
        status = 'Partial';
        score = Math.round((qPassed / qTotal) * 100);
        partialCount++;
      } else {
        status = 'Failed';
        score = 0;
    ...
    const halfwayScore = totalTestCasesCount > 0
    ...
    const report: InterviewReport = {
      ...
      summary: `[TERMINATED: CHEATING DETECTED] ${langLabel} Coding Assessment (${setNameLabel}) was prematurely terminated by AI Proctor due to maximum integrity violations. Halfway Score: ${halfwayScore}%. Reason: ${termReason}`,
      strengths: [],
      weaknesses: [
    ...
    // Persist proctor terminated session to Supabase
    ...
    saveCodingSessionToDb(sessionToSave).catch((err) => {
      console.warn('Could not save proctor-terminated coding session to Supabase:', err);
    });
  };
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      ...
      {activeTab === 'coding_room' ? (
        <div className="space-y-6">
          {/* Submitted / Terminated Banner */}
          {isRoundSubmitted && submittedReport && (
            <div
              className={`border rounded-2xl p-5 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-xl ${
                submittedReport.proctor_terminated || isProctorTerminated
            ...
            >
              <div className="flex items-center space-x-3">
                <div
                  className={`p-2.5 border rounded-xl shrink-0 ${
                    submittedReport.proctor_terminated || isProctorTerminated
                ...
                <div>
                  <h3 className="font-bold text-white text-sm">
                    {submittedReport.proctor_terminated || isProctorTerminated
                      ? 'Assessment Terminated: Cheating / Strike Limit Exceeded'
                      : 'Coding Assessment Submitted & Locked'}
                  </h3>
                  <p className="text-xs text-slate-300 mt-0.5">
                    Candidate: <strong className="text-white">{submittedReport.candidateName}</strong>
                    {submittedReport.proctor_termination_reason && (
                      <span className="block text-[11px] text-rose-300 mt-1 font-mono">
                        Reason: {submittedReport.proctor_termination_reason}
                      </span>
                    )}
                  </p>
                </div>
              ...
              <div className="flex items-center space-x-3 shrink-0">
                {onViewReport && (
                  <button
                    ...
                    className={`px-4 py-2 text-white rounded-xl text-xs font-semibold shadow-lg transition-all flex items-center space-x-2 ${
                      submittedReport.proctor_terminated || isProctorTerminated
          ...
          {!isAssessmentStarted && !isRoundSubmitted ? (
            <div className="bg-[#0F0F12] border border-white/10 rounded-2xl p-6 sm:p-8 space-y-6 max-w-2xl mx-auto shadow-2xl">
              ...
              <div className="space-y-3 max-w-md mx-auto pt-2 bg-white/5 p-4 rounded-2xl border border-white/10">
                ...
                {/* AI Proctor Mode Toggle */}
                <div className="pt-2 border-t border-white/10 space-y-2">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                      <div
                        className={`p-1.5 rounded-lg border transition-all ${
                          isProctorModeEnabled
                      ...
                      <div>
                        <label
                          htmlFor="coding-proctor-mode-toggle"
                          className="text-xs font-bold text-white block cursor-pointer"
                        >
                          AI Proctor Mode
                        </label>
                        <span className="text-[10px] text-slate-400">
                          Local MediaPipe face & gaze integrity check (3 strikes + 4th ends test)
                        </span>
                      </div>
                    </div>
                    ...
                    <label className="relative inline-flex items-center cursor-pointer select-none">
                      <input
                        id="coding-proctor-mode-toggle"
                        type="checkbox"
                        checked={isProctorModeEnabled}
                        onChange={(e) => setIsProctorModeEnabled(e.target.checked)}
                  ...
                  <div className="flex items-center justify-between text-[10px] pt-1 border-t border-white/5 font-mono">
                    ...
                    {isProctorModeEnabled ? (
                      <span className="text-emerald-400 font-semibold flex items-center space-x-1">
                        ...
                        <span>Active (Webcam + Face Tracking)</span>
                      </span>
                    ) : (
                      <span className="text-slate-500 font-medium">Off (Standard Mode)</span>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="pt-2 text-center space-y-3">
                {createAssessmentError && (
                  <div className="p-3.5 bg-rose-500/10 border border-rose-500/30 rounded-xl text-xs text-rose-300 text-left flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 max-w-md mx-auto">
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        navigator.clipboard.writeText(CODING_SUPABASE_MIGRATION_SQL);
                        setHasCopiedMigrationSql(true);
                        setTimeout(() => setHasCopiedMigrationSql(false), 2500);
                      }}
                      className="px-2.5 py-1 bg-rose-500/20 hover:bg-rose-500/30 border border-rose-500/40 rounded-lg text-[11px] font-semibold text-rose-200 flex items-center space-x-1.5 transition-all whitespace-nowrap cursor-pointer shrink-0 self-end sm:self-auto"
                    >
                      {hasCopiedMigrationSql ? (
                        <>
                  ...
                  {(currentQuestion.sampleInput || currentQuestion.sampleOutput) && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-2 text-[11px]">
                      <div className="bg-black/40 p-2 rounded-lg border border-white/5">
                        ...
                        <pre className="text-indigo-300 font-mono whitespace-pre-wrap break-all text-xs max-h-28 overflow-y-auto">{currentQuestion.sampleInput || '(none)'}</pre>
                      </div>
                      <div className="bg-black/40 p-2 rounded-lg border border-white/5">
                        <span className="text-[10px] font-bold text-slate-500 uppercase block mb-1">Sample Output</span>
                        <pre className="text-emerald-300 font-mono whitespace-pre-wrap break-all text-xs max-h-28 overflow-y-auto">{currentQuestion.sampleOutput || '(none)'}</pre>
                      </div>
                    </div>
                  )}
                </div>
                        ...
                        >
                          ...
                          {!res.isHidden && (
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 pt-1 font-mono text-[11px]">
                              <div>
                                <span className="text-[10px] text-slate-400 block uppercase mb-0.5">Input</span>
                                <pre className="text-slate-200 bg-black/40 px-2.5 py-1.5 rounded block whitespace-pre-wrap break-all max-h-28 overflow-y-auto border border-white/5">
                                  {res.input || '(empty)'}
                                </pre>
                              </div>
                              <div>
                                <span className="text-[10px] text-slate-400 block uppercase mb-0.5">Expected</span>
                                <pre className="text-slate-200 bg-black/40 px-2.5 py-1.5 rounded block whitespace-pre-wrap break-all max-h-28 overflow-y-auto border border-white/5">
                                  {res.expectedOutput}
                                </pre>
                              </div>
                              <div>
                                <span className="text-[10px] text-slate-400 block uppercase mb-0.5">Actual Output</span>
                                <pre className="text-slate-200 bg-black/40 px-2.5 py-1.5 rounded block whitespace-pre-wrap break-all max-h-28 overflow-y-auto border border-white/5">
          ...
          <div className="bg-[#121216] border border-white/10 rounded-2xl max-w-2xl w-full p-6 space-y-4 shadow-2xl text-slate-100 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-white/10 pb-3">
              <div>
                ...
                <p className="text-[11px] text-slate-400 mt-0.5">Define question details, multiline test cases, and hidden verification suites.</p>
              </div>
              <button
                type="button"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-1">
                  ...
                  <textarea
                    ...
                    className="w-full px-3 py-2 bg-black/40 border border-white/10 rounded-xl text-white focus:outline-none focus:border-indigo-500 font-mono text-xs resize-y"
                  />
                </div>
                <div className="space-y-1">
                  <div className="flex items-center justify-between">
              ...
              <div className="space-y-3 border-t border-white/10 pt-3">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-[11px] text-slate-400">
                      Multi-line stdin input and expected output supported. Use Enter or Shift+Enter freely.
                    </p>
                  </div>
                ...
                <div className="space-y-3">
                  {newTestCases.map((tc, idx) => (
                    <div
                      key={tc.id}
                      className="bg-black/50 p-3.5 rounded-xl border border-white/10 space-y-2.5"
                    >
                      <div className="flex items-center justify-between border-b border-white/5 pb-2">
                        <div className="flex items-center space-x-2">
                          <span className="font-semibold text-xs text-white">Test Case #{idx + 1}</span>
                          <span className="text-[10px] text-slate-400 font-mono">
                            {tc.isHidden ? '(Hidden from candidate)' : '(Visible to candidate)'}
                          </span>
                        </div>
                        <div className="flex items-center space-x-3">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div className="space-y-1">
                          <div className="flex items-center justify-between">
                            <label className="text-slate-300 font-medium text-[11px]">Input (stdin)</label>
                            <span className="text-[10px] text-slate-500 font-mono">Enter / Shift+Enter</span>
                          </div>
                          <textarea
                            rows={3}
                            placeholder={'8\n10 5 2 7 1 9 -5 3\n15'}
                            ...
                            className="w-full px-3 py-2 bg-black/60 border border-white/10 rounded-lg text-white font-mono text-xs focus:outline-none focus:border-indigo-500 resize-y"
                          />
                        </div>

                        <div className="space-y-1">
                          <div className="flex items-center justify-between">
                            <label className="text-slate-300 font-medium text-[11px]">Expected Output</label>
                            <span className="text-[10px] text-slate-500 font-mono">Exact stdout</span>
                          </div>
                          <textarea
                            rows={3}
                            placeholder={'4'}
      ...
      {/* Real-time AI Face Proctor Widget for Coding Session */}
      {isAssessmentStarted && !isRoundSubmitted && isProctorModeEnabled && (
        <ProctorCameraWidget
          isEnabled={isProctorModeEnabled && isAssessmentStarted && !isRoundSubmitted}
          maxStrikes={4}
