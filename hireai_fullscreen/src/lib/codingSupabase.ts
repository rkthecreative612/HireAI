...
/**
 * Migration SQL script for existing databases to add AI Proctoring support
 */
export const CODING_SUPABASE_MIGRATION_SQL = `
-- Run this in Supabase SQL Editor to add AI Proctoring support to existing tables:
ALTER TABLE coding_assessments ADD COLUMN IF NOT EXISTS proctor_enabled BOOLEAN DEFAULT true;
ALTER TABLE coding_sessions ADD COLUMN IF NOT EXISTS proctor_enabled BOOLEAN DEFAULT true;
ALTER TABLE coding_sessions ADD COLUMN IF NOT EXISTS proctor_events JSONB DEFAULT '[]'::jsonb;
ALTER TABLE coding_sessions ADD COLUMN IF NOT EXISTS proctor_terminated BOOLEAN DEFAULT false;
ALTER TABLE coding_sessions ADD COLUMN IF NOT EXISTS proctor_termination_reason TEXT;
ALTER TABLE coding_sessions ADD COLUMN IF NOT EXISTS proctor_strikes_count INTEGER DEFAULT 0;
`;
...
CREATE TABLE IF NOT EXISTS coding_sessions (
  ...
  proctor_enabled BOOLEAN DEFAULT true,
  proctor_events JSONB DEFAULT '[]'::jsonb,
  proctor_terminated BOOLEAN DEFAULT false,
  proctor_termination_reason TEXT,
  proctor_strikes_count INTEGER DEFAULT 0,
  created_at TIMESTAMPTZ DEFAULT now(),
...
-- Migration / Upgrade for existing databases (adds proctoring columns if missing)
ALTER TABLE coding_assessments ADD COLUMN IF NOT EXISTS proctor_enabled BOOLEAN DEFAULT true;
ALTER TABLE coding_sessions ADD COLUMN IF NOT EXISTS proctor_enabled BOOLEAN DEFAULT true;
ALTER TABLE coding_sessions ADD COLUMN IF NOT EXISTS proctor_events JSONB DEFAULT '[]'::jsonb;
ALTER TABLE coding_sessions ADD COLUMN IF NOT EXISTS proctor_terminated BOOLEAN DEFAULT false;
...
export async function saveCodingSessionToDb(
  ...
  try {
    const proctorAuditMeta = {
      _proctor_meta: true,
      proctor_enabled: session.proctor_enabled,
      proctor_events: session.proctor_events,
      proctor_terminated: session.proctor_terminated,
      proctor_termination_reason: session.proctor_termination_reason,
      proctor_strikes_count: session.proctor_strikes_count,
    };

    // Embed audit info in code_submissions so data is never lost even if table has no proctor columns
    const codeSubmissionsWithAudit = {
      ...(session.code_submissions || {}),
      __proctor_audit__: JSON.stringify(proctorAuditMeta)
    };
    ...
    const fullPayload: Record<string, any> = {
      ...
      code_submissions: codeSubmissionsWithAudit,
      question_results: session.question_results || [],
      proctor_enabled: session.proctor_enabled || false,
      proctor_events: session.proctor_events || [],
      proctor_terminated: session.proctor_terminated || false,
      proctor_termination_reason: session.proctor_termination_reason || null,
      proctor_strikes_count: session.proctor_strikes_count || 0,
      created_at: session.created_at || new Date().toISOString(),
      completed_at: session.completed_at || null,
    };

    let { error } = await client.from(CODING_TABLE_SESSIONS).upsert(fullPayload);

    // If proctor columns are not in the Supabase schema cache, retry without them
    if (error && (error.message?.includes('proctor_') || error.message?.includes('schema cache'))) {
      console.warn('Supabase coding_sessions table missing proctor columns. Retrying upsert without proctor columns (audit metadata preserved in code_submissions)...', error.message);
      ...
      const fallbackRes = await client.from(CODING_TABLE_SESSIONS).upsert(fallbackPayload);
...
export async function updateCodingSessionInDb(
  ...
  try {
    ...
    // Optional proctor updates
    if (updates.proctor_strikes_count !== undefined) payload.proctor_strikes_count = updates.proctor_strikes_count;
    if (updates.proctor_events !== undefined) payload.proctor_events = updates.proctor_events;
    ...
    // If proctor columns don't exist in schema, retry without them
    if (error && (error.message?.includes('proctor_') || error.message?.includes('schema cache'))) {
      delete payload.proctor_strikes_count;
      delete payload.proctor_events;
...
export async function fetchCodingSessionsFromDb(): Promise<CodingSessionRecord[]> {
  ...
  try {
    ...
    return data.map((row: any) => {
      let proctorMeta: any = {};
      if (row.code_submissions && typeof row.code_submissions.__proctor_audit__ === 'string') {
        try {
          proctorMeta = JSON.parse(row.code_submissions.__proctor_audit__);
        } catch {}
      }

      const proctor_enabled = row.proctor_enabled !== undefined && row.proctor_enabled !== null
        ? Boolean(row.proctor_enabled)
        : Boolean(proctorMeta.proctor_enabled);

      const proctor_events = Array.isArray(row.proctor_events) && row.proctor_events.length > 0
        ? row.proctor_events
        : (Array.isArray(proctorMeta.proctor_events) ? proctorMeta.proctor_events : []);

      const proctor_terminated = row.proctor_terminated !== undefined && row.proctor_terminated !== null
        ? Boolean(row.proctor_terminated)
        : Boolean(proctorMeta.proctor_terminated);

      const proctor_termination_reason = row.proctor_termination_reason || proctorMeta.proctor_termination_reason || undefined;
      const proctor_strikes_count = Number(row.proctor_strikes_count) || Number(proctorMeta.proctor_strikes_count) || 0;
      ...
      return {
        ...
        proctor_events,
        proctor_terminated,
        proctor_termination_reason,
        proctor_strikes_count,
...
export function codingSessionToInterviewReport(session: CodingSessionRecord): InterviewReport {
  ...
  return {
    ...
    recommendation,
    summary: session.proctor_terminated
      ? `[PROCTOR TERMINATED] Candidate ${session.candidate_name} had the assessment automatically terminated due to excessive proctoring violations (${session.proctor_strikes_count || 4} strikes). Reason: ${session.proctor_termination_reason || 'Continuous gaze/face violation'}. Halfway score at termination: ${finalScore}%. Passed ${passedTests}/${totalTests} test cases.`
      : `Candidate ${session.candidate_name} completed ${session.language === 'python' ? 'Python' : 'Java'} coding assessment (${session.set_name}). Passed ${passedTests} of ${totalTests} test cases (${finalScore}%). Solved ${solvedCount} of ${qResults.length} questions. Overall evaluation: ${recommendation}.`,
    strengths: qResults.filter((q) => q.status === 'Solved').map((q) => `Fully solved "${q.questionTitle}" passing all test assertions.`),
    weaknesses: [
      ...(session.proctor_terminated ? [`Proctor Integrity Violation: Test terminated at 4 strikes (${session.proctor_termination_reason || 'Proctor violation'}).`] : []),
      ...qResults.filter((q) => q.status !== 'Solved').map((q) => `${q.status === 'Not Attempted' ? 'Did not attempt' : 'Partial / Failed'} "${q.questionTitle}" (${q.passedCount}/${q.totalTestCases} test cases passed).`)
    ],
    categoryBreakdown: [
      {
        category: 'domain',
    ...
    followUpQuestionsForInterviewer: [
      ...(session.proctor_terminated ? ['Inquire about multiple camera/screen violations flagged by the automated AI proctoring system during the test.'] : []),
    ...
    proctor_termination_reason: session.proctor_termination_reason,
    proctor_strikes_count: session.proctor_strikes_count,
    proctorEvents: session.proctor_events || [],
    proctorTerminated: session.proctor_terminated || false,
    proctorTerminationReason: session.proctor_termination_reason,
    proctorStrikesCount: session.proctor_strikes_count,
  };
}
...
export async function createCodingAssessmentInDb(
  ...
  try {
    const isProctor = assessment.proctor_enabled ?? true;

    // Attach _proctor_enabled to the first question so it is preserved even if the table has no proctor_enabled column
    const questionsWithMeta = Array.isArray(assessment.questions)
      ? assessment.questions.map((q, idx) => (idx === 0 ? { ...q, _proctor_enabled: isProctor } : q))
    ...
    const fullPayload: Record<string, any> = {
      ...
      proctor_enabled: isProctor,
      created_at: assessment.created_at || new Date().toISOString(),
    };

    let { error } = await client.from(CODING_TABLE_ASSESSMENTS).upsert(fullPayload);

    // If proctor_enabled column is not in the schema cache, retry gracefully without it
    if (error && (error.message?.includes('proctor_enabled') || error.message?.includes('schema cache'))) {
      console.warn('Supabase coding_assessments missing proctor_enabled column. Retrying upsert without column (preserved in questions)...', error.message);
      ...
      const fallbackRes = await client.from(CODING_TABLE_ASSESSMENTS).upsert(fallbackPayload);
      error = fallbackRes.error;
    }
...
export async function fetchCodingAssessmentByToken(
  ...
  try {
    ...
    // Extract proctor_enabled from row or embedded metadata in questions[0]
    let proctorEnabled = true;
    if (data.proctor_enabled !== undefined && data.proctor_enabled !== null) {
      proctorEnabled = Boolean(data.proctor_enabled);
    } else if (Array.isArray(data.questions) && data.questions[0] && (data.questions[0] as any)._proctor_enabled !== undefined) {
      proctorEnabled = Boolean((data.questions[0] as any)._proctor_enabled);
    }
