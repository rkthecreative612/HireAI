...
export async function fetchCodingSessionsFromDb(): Promise<CodingSessionRecord[]> {
  ...
  try {
    ...
    return data.map((row: any) => ({
      ...
      proctor_enabled: Boolean(row.proctor_enabled),
      proctor_events: Array.isArray(row.proctor_events) ? row.proctor_events : [],
      proctor_terminated: Boolean(row.proctor_terminated),
      proctor_termination_reason: row.proctor_termination_reason || undefined,
      proctor_strikes_count: Number(row.proctor_strikes_count) || 0,
...
export async function createCodingAssessmentInDb(
  ...
  try {
    const { error } = await client.from(CODING_TABLE_ASSESSMENTS).upsert({
      ...
      proctor_enabled: assessment.proctor_enabled || false,
...
export async function fetchCodingAssessmentByToken(
  ...
  try {
    ...
    const record: CodingAssessmentRecord = {
      ...
      proctor_enabled: Boolean(data.proctor_enabled),
